    ...
    methods: {
        ...
        search(flag, pageCode, pageSize) {
            this.$http.post(api.category.findByPage(flag, pageSize, pageCode)).then(result => {
                if (flag == 'category') {
                    this.entity.category = result.body.data.list;
                    this.pageConf.totalPage = result.body.data.total;
                }
                if (flag == 'tags') {
                    this.entity.tags = result.body.data.list;
        ...
        t_handleSizeChange(val) {
            this.pageConf.pageSize = val;
            this.search('tags', this.pageConf.t_pageCode, this.pageConf.pageSize);
        },
        //当前页改变时触发的函数
        ...
        //更新按钮
        handleEdit(flag, id, name) {
            this.dialogVisible = true;
            //查询当前id对应的数据
            this.editor = {id:id,name:name};
